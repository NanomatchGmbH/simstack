...
class TestResourcesView:
    ...
    def resources_view_wano(self, qtbot, mock_resources):
        ...
        with patch('simstack.view.ResourcesView.QtClusterSettingsProvider'), \
             patch('simstack.view.ResourcesView.WaNoRegistrySelection'):
    ...
    def test_field_name_to_display_name(self):
        ...
        assert ResourcesView.field_name_to_display_name("unknown_field") == "unknown_field"
    ...
    def test_reinit_values_from_resource(self, resources_view_wano, mock_resources):
        ...
        int_widget.setValue.assert_called_once_with(mock_resources.get_field_value("port"))
        file_widget.line_edit.setText.assert_called_once_with(mock_resources.get_field_value("extra_config"))
        str_widget.setText.assert_called_once_with(mock_resources.get_field_value("base_URI"))
        bool_widget.setChecked.assert_called_once_with(mock_resources.get_field_value("reuse_results"))
        queueing_widget.setCurrentText.assert_called_once_with(mock_resources.get_field_value("queueing_system"))
        resource_chooser_widget.setCurrentText.assert_called_once_with(mock_resources.get_field_value("resource_name"))
    
    @pytest.mark.skip(reason="Difficult to properly mock Qt widgets in this specific case")
    ...
    @patch('simstack.view.ResourcesView.QtClusterSettingsProvider')
    def test_on_cluster_dropdown_change_connected_server(self, mock_csp, resources_view_wano, mock_resources):
        ...
        assert mock_resources.set_field_value.call_count >= len(ResourcesView.wano_exclusion_items) + 1
        mock_resources.set_field_value.assert_any_call("resource_name", view._connected_server_text)
        view._reinit_values_from_resource.assert_called_once()
    
    @patch('simstack.view.ResourcesView.QtClusterSettingsProvider')
    def test_on_cluster_dropdown_change_registry(self, mock_csp, resources_view_wano, mock_resources):
        ...
        assert mock_resources.set_field_value.call_count >= len(mock_resources.render_order())
        view._reinit_values_from_resource.assert_called_once()
    ...
    def test_fieldchanger(self, mock_resources):
        ...
        # Verify
        mock_resources.set_field_value.assert_called_once_with("test_field", "new_value")