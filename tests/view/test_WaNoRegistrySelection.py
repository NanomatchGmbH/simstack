...
@pytest.mark.skip(reason="WaNoRegistrySelection tests need to be completely rewritten to use proper mocks")
class TestWaNoRegistrySelection:
    ...
    def registry_selection(self, qtbot):
        ...
        with patch('simstack.view.WaNoRegistrySelection.QtClusterSettingsProvider') as mock_provider, \
             patch('simstack.view.WaNoRegistrySelection.os.path.dirname') as mock_dirname, \
             patch('simstack.view.WaNoRegistrySelection.os.path.realpath') as mock_realpath, \
             patch('simstack.view.WaNoRegistrySelection.os.path.join') as mock_join:
            ...
            with patch.object(WaNoRegistrySelection, '_WaNoRegistrySelection__init_ui'), \
                 patch.object(WaNoRegistrySelection, '_WaNoRegistrySelection__connect_signals'), \
                 patch.object(WaNoRegistrySelection, 'setStatus'), \
                 patch.object(WaNoRegistrySelection, '_WaNoRegistrySelection__update_status'):
                ...
                widget._WaNoRegistrySelection__status = WaNoRegistrySelection.CONNECTION_STATES.disconnected
    ...
    def test_init(self, registry_selection):
        ...
        assert registry_selection._WaNoRegistrySelection__status == WaNoRegistrySelection.CONNECTION_STATES.disconnected
    ...
    def test_set_status_connected(self, registry_selection):
        ...
        with patch('simstack.view.WaNoRegistrySelection.QPixmap') as mock_pixmap, \
             patch('simstack.view.WaNoRegistrySelection.QIcon') as mock_icon:
            ...
            registry_selection.setStatus(WaNoRegistrySelection.CONNECTION_STATES.connected)
            
            # Verify status was updated
            assert registry_selection._WaNoRegistrySelection__status == WaNoRegistrySelection.CONNECTION_STATES.connected
            
            # Verify icon was updated
            mock_pixmap.assert_called_once()
            mock_icon.assert_called_once_with(pixmap_instance)
            registry_selection.isConnectedIcon.setIcon.assert_called_once_with(icon_instance)
            
            # Verify text was updated
            registry_selection.isConnectedIcon.setText.assert_called_once_with("Disconnect")
    ...
    def test_set_status_connecting(self, registry_selection):
        ...
        with patch('simstack.view.WaNoRegistrySelection.QPixmap') as mock_pixmap, \
            ...
            registry_selection.setStatus(WaNoRegistrySelection.CONNECTION_STATES.connecting)
            
            # Verify status was updated
            assert registry_selection._WaNoRegistrySelection__status == WaNoRegistrySelection.CONNECTION_STATES.connecting
            
            # Verify text was updated
            registry_selection.isConnectedIcon.setText.assert_called_once_with("[Connecting]")
    ...
    def test_set_status_disconnected(self, registry_selection):
        ...
        with patch('simstack.view.WaNoRegistrySelection.QPixmap') as mock_pixmap, \
            ...
            registry_selection.setStatus(WaNoRegistrySelection.CONNECTION_STATES.disconnected)
            
            # Verify status was updated
            assert registry_selection._WaNoRegistrySelection__status == WaNoRegistrySelection.CONNECTION_STATES.disconnected
            
            # Verify text was updated
            registry_selection.isConnectedIcon.setText.assert_called_once_with("Connect")
    ...
    def test_update_registries(self, registry_selection):
        ...
        assert registry_selection._WaNoRegistrySelection__emit_signal is True  # Should be reset to True after update
    ...
    def test_new_update_registries(self, registry_selection):
        ...
        # Mock methods
        registry_selection.registryComboBox.currentText = MagicMock(return_value="registry1")
        registry_selection.registryComboBox.clear = MagicMock()
        registry_selection.registryComboBox.addItem = MagicMock()
        ...
        registry_selection.registryComboBox.clear.assert_called_once()
        assert registry_selection.registryComboBox.addItem.call_count == 3
        registry_selection.registryComboBox.setCurrentText.assert_called_once_with("registry1")
    ...
    def test_on_selection_changed(self, registry_selection, qtbot):
        ...
        registry_selection.registrySelectionChanged.emit.assert_called_once_with("new_registry")
        ...
        registry_selection._WaNoRegistrySelection__on_selection_changed("another_registry")
    ...
    def test_on_button_clicked_connected(self, registry_selection, qtbot):
        ...
        # Set status to connected
        registry_selection._WaNoRegistrySelection__status = WaNoRegistrySelection.CONNECTION_STATES.connected
    ...
    def test_on_button_clicked_disconnected(self, registry_selection, qtbot):
        ...
        # Set status to disconnected
        registry_selection._WaNoRegistrySelection__status = WaNoRegistrySelection.CONNECTION_STATES.disconnected
        
        # Mock current registry
        registry_selection.registryComboBox.currentText = MagicMock(return_value="current_registry")
        
        # Call method directly
        registry_selection._WaNoRegistrySelection__on_button_clicked()
        
        # Verify connect signal was emitted with correct argument
        registry_selection.connect_registry.emit.assert_called_once_with("current_registry")
    ...
    def test_on_button_clicked_connecting(self, registry_selection, qtbot):
        ...
        # Set status to connecting
        registry_selection._WaNoRegistrySelection__status = WaNoRegistrySelection.CONNECTION_STATES.connecting
