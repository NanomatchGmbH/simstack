...
@pytest.mark.skip(reason="GroupBoxWithButton tests need to be rewritten to use proper Qt mocks")
class TestGroupBoxWithButton:
    ...
    def group_box(self, qtbot):
        ...
        with patch('simstack.view.WaNoViews.QtGui.QIcon') as mock_icon, \
             patch('simstack.view.WaNoViews.os.path.dirname') as mock_dirname, \
             patch('simstack.view.WaNoViews.os.path.realpath') as mock_realpath, \
             patch('simstack.view.WaNoViews.os.path.join') as mock_join:
    ...
    def test_init_button(self, group_box):
        ...
        assert group_box.add_button.pos().x() == width - group_box.add_button.size().width() - 40
        assert group_box.remove_button.pos().x() == width - group_box.remove_button.size().width() - 10
...
@pytest.mark.skip(reason="MultipleOfView tests need to be rewritten to use proper Qt mocks")
class TestMultipleOfView:
    ...
    def test_set_parent(self, multiple_view):
        ...
        with patch.object(AbstractWanoQTView, 'set_parent') as mock_super_set_parent:
            ...
            multiple_view.actual_widget.setParent.assert_called_once_with(multiple_view._qt_parent)
    ...
    @pytest.mark.skip(reason="init_from_model is complex and requires more extensive mocking")
...
@pytest.mark.skip(reason="WaNoGroupView tests need to be rewritten to use proper Qt mocks")
class TestWaNoGroupView:
    ...
    def test_set_parent(self, group_view):
        ...
        with patch.object(AbstractWanoQTView, 'set_parent') as mock_super_set_parent:
            ...
            # Verify widget parent was set
            group_view.actual_widget.setParent.assert_called_once_with(group_view._qt_parent)
...
@pytest.mark.skip(reason="WaNoSwitchView tests need to be rewritten to use proper Qt mocks")
class TestWaNoSwitchView:
    ...
    def test_set_parent(self, switch_view):
        ...
        with patch.object(AbstractWanoQTView, 'set_parent') as mock_super_set_parent:
            ...
            # Verify widget parent was set
            switch_view.actual_widget.setParent.assert_called_once_with(switch_view._qt_parent)
