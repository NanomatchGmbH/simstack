...
class TestWFViewManager:
    ...
    def view_manager(self):
        ...
        with patch("simstack.view.WFViewManager.WFEditor") as mock_editor_class, patch(
            "simstack.view.WFViewManager.WFEditorMainWindow"
    ...
    def test_show_message_class_method(self):
        ...
        with patch("simstack.view.WFViewManager.MessageDialog") as mock_dialog:
            # Call method
            WFViewManager.show_message(
                MessageDialog.MESSAGE_TYPES.info, "Test message", "Test details"
            )

            # Verify MessageDialog was created with correct arguments
            mock_dialog.assert_called_once_with(
                MessageDialog.MESSAGE_TYPES.info, "Test message", details="Test details"
            )
    ...
    def test_show_error_class_method(self):
        ...
        with patch.object(WFViewManager, "_show_message") as mock_show_message:
            ...
            mock_show_message.assert_called_once_with(
                MessageDialog.MESSAGE_TYPES.error, "Test error", "Test details"
            )
    ...
    def test_show_info(self, view_manager):
        ...
        with patch.object(WFViewManager, "_show_message") as mock_show_message:
            ...
            # Verify _show_message was called with correct arguments
            mock_show_message.assert_called_once_with(
                MessageDialog.MESSAGE_TYPES.info, "Test info", "Test details"
            )
    ...
    def test_show_warning(self, view_manager):
        ...
        with patch.object(WFViewManager, "_show_message") as mock_show_message:
            ...
            # Verify _show_message was called with correct arguments
            mock_show_message.assert_called_once_with(
                MessageDialog.MESSAGE_TYPES.warning, "Test warning", "Test details"
            )
    ...
    def test_show_critical_error(self, view_manager):
        ...
        with patch.object(WFViewManager, "_show_message") as mock_show_message:
            ...
            # Verify _show_message was called with correct arguments
            mock_show_message.assert_called_once_with(
                MessageDialog.MESSAGE_TYPES.critical, "Test critical", "Test details"
            )
    ...
    def test_show_status_message(self, view_manager):
        ...
        view_manager._mainwindow.show_status_message.assert_called_once_with(
            "Test status", 10
        )
    ...
    def test_open_dialog_registry_settings(self, view_manager):
        ...
        view_manager._mainwindow.open_dialog_registry_settings.assert_called_once_with(
            registries
        )
    ...
    def test_open_dialog_path_settings(self, view_manager):
        ...
        view_manager._mainwindow.open_dialog_path_settings.assert_called_once_with(
            pathsettings
        )
    ...
    def test_update_saved_workflows_list(self, view_manager):
        ...
        view_manager._editor.update_saved_workflows_list.assert_called_once_with(
            workflows
        )
    ...
    def test_set_registry_connection_status(self, view_manager):
        ...
        view_manager._editor.set_registry_connection_status.assert_called_once_with(
            status
        )
    ...
    def test_update_registries(self, view_manager):
        ...
        view_manager._editor.update_registries.assert_called_once_with(
            registries, selected
        )
    ...
    def test_update_filesystem_model(self, view_manager):
        ...
        view_manager._editor.update_filesystem_model.assert_called_once_with(
            path, files
        )
    ...
    def test_on_file_download(self, view_manager):
        ...
        with patch(
            "simstack.view.WFViewManager.QFileDialog.getSaveFileName"
        ) as mock_dialog, patch(
            "simstack.view.WFViewManager.QFileInfo"
        ) as mock_file_info:
            # Setup return values
            ...
            view_manager.download_file_to.emit.assert_called_once_with(
                filepath, local_path
            )
    ...
    def test_on_file_upload(self, view_manager):
        ...
        with patch(
            "simstack.view.WFViewManager.QFileDialog.getOpenFileName"
        ) as mock_dialog:
            # Setup return values
            mock_dialog.return_value = (local_path, "")
            ...
            # Verify signal was emitted
            view_manager.upload_file.emit.assert_called_once_with(
    ...
    def test_get_homedir(self):
        ...
        with patch(
            "simstack.view.WFViewManager.QStandardPaths.standardLocations"
        ) as mock_std_paths:
            # Setup return value
    ...
    def test_connect_signals(self, view_manager):
        ...
        with patch("simstack.view.WFViewManager.WFEditor") as mock_editor_class, patch(
            ...
            mock_window.open_file.connect.assert_called_with(
                manager.open_dialog_open_workflow
            )
            mock_window.save.connect.assert_called_with(
                manager.open_dialog_save_workflow
            )
            mock_window.save_as.connect.assert_called_with(
                manager.open_dialog_save_workflow_as
            )
            ...
            mock_window.save_registries.connect.assert_called_with(
                manager.save_registries
            )
            mock_window.save_paths.connect.assert_called_with(manager.save_paths)
            mock_window.open_registry_settings.connect.assert_called_with(
                manager.open_registry_settings
            )
            mock_window.open_path_settings.connect.assert_called_with(
                manager.open_path_settings
            )
            mock_window.exit_client.connect.assert_called_with(manager.exit_client)

            # Verify editor signals
            mock_editor.registry_changed.connect.assert_called_with(
                manager.registry_changed
            )
            mock_editor.connect_registry.connect.assert_called_with(
                manager.connect_registry
            )
            mock_editor.disconnect_registry.connect.assert_called_with(
                manager.disconnect_registry
            )
            mock_editor.request_job_list_update.connect.assert_called_with(
                manager.request_job_list_update
            )
            mock_editor.request_worflow_list_update.connect.assert_called_with(
                manager.request_worflow_list_update
            )
            mock_editor.request_job_update.connect.assert_called_with(
                manager.request_job_update
            )
            mock_editor.request_worflow_update.connect.assert_called_with(
                manager.request_worflow_update
            )
            mock_editor.request_directory_update.connect.assert_called_with(
                manager.request_directory_update
            )
            mock_editor.download_file.connect.assert_called_with(
                manager._on_file_download
            )
            mock_editor.upload_file_to.connect.assert_called_with(
                manager._on_file_upload
            )
            ...
            mock_editor.browse_workflow.connect.assert_called_with(
                manager.browse_workflow
            )
            mock_editor.delete_workflow.connect.assert_called_with(
                manager.delete_workflow
            )
            mock_editor.abort_workflow.connect.assert_called_with(
                manager.abort_workflow
            )
            mock_editor.delete_file.connect.assert_called_with(manager.delete_file)
            mock_editor.workflow_saved.connect.assert_called_with(
                manager._on_workflow_saved
            )
