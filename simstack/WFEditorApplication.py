...
class WFEditorApplication:
    ...
    def _on_error(self, base_uri, operation, error, error_msg=None):
        ...
        if error == uerror.REGISTRY_NOT_CONNECTED.value:
            ...
            uops.UPDATE_WF_LIST.value,
            uops.UPDATE_WF_JOB_LIST.value,
            uops.UPDATE_DIR_LIST.value,
            uops.RUN_WORKFLOW_JOB.value,
        ]:
            func = self._on_run_single_job_error
    ...
    def _on_open_path_settings(self):
        self._view_manager.open_dialog_path_settings(
            self.__settings.get_path_settings()
        )
    ...
    def _on_fs_worflow_list_update_request(self):
        ...
        self._connector.update_workflow_list(
            registry_name, (self._on_workflow_list_updated, (), {})
        )
    ...
    def _on_fs_job_update_request(self, path):
        registry_name = self._get_current_registry_name()
        self._connector.update_dir_list(
            registry_name, path, (self._on_fs_list_updated, (), {})
        )

    def _on_fs_workflow_update_request(self, wfid):
        # self._logger.debug("Querying %s from Registry" % wfid)
        registry_name = self._get_current_registry_name()
        self._connector.update_workflow_job_list(
            registry_name, wfid, (self._on_fs_list_updated, (), {})
        )
    ...
    def __on_download_update(self, uri, localdest, progress, total):
        print(
            "\t%6.2f (%6.2f / %6.2f)\t%s"
            % (progress / total * 100.0, progress, total, uri)
        )
    ...
    def _on_file_deleted(self, base_uri, status, err, to_del=""):
        ...
        if err != ErrorCodes.NO_ERROR:
            self._view_manager.show_error(
                ...
                "Registry returned status: %s." % (to_del, str(status.name))
            )
        else:
            if not (to_update is None or to_update == ""):
                self._logger.debug(
                    "Issuing filesystem update for '%s' after delete." % to_update
                )
    ...
    def _on_fs_delete_file(self, filename):
        ...
        self._connector.delete_file(
            registry_name, filename, (self._on_file_deleted, (), {"to_del": filename})
        )
    ...
    def _on_job_deleted(self, base_uri, status, err, job=""):
        if err == ErrorCodes.NO_ERROR:
            ...
            self._view_manager.show_error(
                "Failed to delete job '%s'\n"
                "Registry returned status: %s." % (job, str(status.name))
            )
    ...
    def _on_workflow_deleted(self, base_uri, status, err, workflow=""):
        if err == ErrorCodes.NO_ERROR:
            ...
            self._view_manager.show_error(
                ...
                "Registry returned status: %s." % (workflow, str(status.name))
            )
    ...
    def _on_fs_delete_workflow(self, workflow):
        registry_name = self._get_current_registry_name()
        self._connector.delete_workflow(
            registry_name,
            workflow,
    ...
    def _on_fs_abort_workflow(self, workflow):
        registry_name = self._get_current_registry_name()
        self._connector.abort_workflow(
            registry_name,
            workflow,
    ...
    def _cb_connect(self, base_uri, error, status, registry_name=None):
        ...
        if error == ErrorCodes.NO_ERROR:
            ...
            self._logger.error(
                "Failed to connect to registry: %s, %s" % (str(status), str(error))
            )
            self._view_manager.show_error(
                "Failed to connect to registry '%s'\n"
                "Connection returned status: %s." % (registry_name, status)
            )
    ...
    def _disconnect_remote(self, no_status_update=False):
        ...
        self._connector.disconnect_registry(
            registry_name, (self._cb_disconnect, (), {"registry_name": registry_name})
        )
    ...
    def _connect_remote(self, registry_name, no_status_update=False):
        ...
        self._connector.connect_registry(
            registry_name, (self._cb_connect, (), {"registry_name": registry_name})
        )
    ...
    def _run(self):
        ...
        try:
            ...
            self._view_manager.show_error(
                "File not found during Workflow rendering. Error was: %s" % e
            )
            return
            ...
            traceback.print_exc()
            self._view_manager.show_error(
                "Please save workflow before submit. Error was: %s" % e
            )
            return
        except WorkflowSubmitError as e:
            errormessage = (
                "Error during workflow submit. Error was:\n\n -----\n\n%s" % e
            )
            import traceback
    ...
    def _get_current_workflow_uri(self):
        registry = self._get_current_registry()
        return "" if registry is None else registry[SETTING_KEYS["registry.workflows"]]

    def _update_wanos(self):
        self.__load_wanos_from_repo(self.__settings.get_value(SETTING_KEYS["wanoRepo"]))
    ...
    def _connect_signals(self):
        ...
        self._view_manager.open_registry_settings.connect(
            self._on_open_registry_settings
        )
        ...
        self._view_manager.request_job_list_update.connect(
            self._on_fs_job_list_update_request
        )
        self._view_manager.request_worflow_list_update.connect(
            self._on_fs_worflow_list_update_request
        )
        self._view_manager.request_job_update.connect(self._on_fs_job_update_request)
        self._view_manager.request_worflow_update.connect(
            self._on_fs_workflow_update_request
        )
        self._view_manager.request_directory_update.connect(
            self._on_fs_directory_update_request
        )
        self._view_manager.request_saved_workflows_update.connect(
            self._on_saved_workflows_update_request
        )
