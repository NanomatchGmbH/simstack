...
class AbstractSettings(object):
    ...
    def dump_to_file(self, filename):
        with open(filename, "w") as outfile:
            outfile.write(
                yaml.safe_dump(self.settings_container, default_flow_style=False)
            )
    ...
    def _set_defaults(self):
        raise NotImplementedError(
            "Abstract Virtual Method, please implement in child class"
        )
        return
    ...
    def set_value(self, valuename, value):
        splitname = [
            x for x in map(self._cast_string_to_correct_type, valuename.split("."))
        ]
        current_dict = self.settings_container
        # in order to prevent changes in settings already saved, check validity
        # of the specified keys first.
        for key in splitname:
            if current_dict is None or not (
                isinstance(current_dict, dict) or isinstance(current_dict, list)
            ):
        ...
        for key, nextkey in zip(splitname[:-1], splitname[1:]):
            if isinstance(key, str) and key not in current_dict:
        ...
        if isinstance(current_dict, list):
            if key >= 0 and key < len(current_dict):
                ...
                raise IndexError(
                    "Out of range. You can only append consecutive list items."
                )
        ...
        for argtuple in args:
            ...
            try:
                ...
                for field in splitset[0][:-1]:
                    if (field not in current_dict) and createdicts:
                        current_dict[field] = {}
                    if field not in current_dict:
                        raise KeyError(
                            "The settings module did not contain %s. Please check your input."
                            % argtuple
                        )
                    current_dict = current_dict[field]
                final_key = splitset[0][-1]
                if final_key in current_dict or createdicts:
                    current_dict[final_key] = self._cast_string_to_correct_type(
                        splitset[1]
                    )
                else:
                    raise KeyError(
    ...
    def _recursive_load(self, load, keystring=False):
        if isinstance(load, dict) or isinstance(load, list):
            items = load.items() if isinstance(load, dict) else enumerate(load)
            for key, value in items:
                new_keystr = (
                    "%s.%s" % (str(keystring), str(key)) if keystring else str(key)
                )
            ...
            raise RuntimeError("Parsing error: key: %s, load: %s" % (keystring, load))
    ...
    def _set_defaults(self):
        defaults = [
            ...
            (
                "Box.Ly",
                "sameas:Box.Lx",
                "Half box length, y direction [Angstrom] By default same as Box.Lx",
            ),
            ...
            (
                "settings.filename",
                "TestSettings.yml",
                "Filename of the file the chosen settings are written to",
            ),
        ]
